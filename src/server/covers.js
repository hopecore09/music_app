...
export async function generateCover(seed, { title, artist }) {
  ...
  const svg = `<svg width="400" height="400" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="200" y="180" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="${ts}" font-weight="700" fill="#ffffff">${esc(title)}</text>
    <text x="200" y="235" text-anchor="middle" font-family="Arial,Helvetica,sans-serif" font-size="${as}" font-weight="300" fill="rgba(255,255,255,0.9)">${esc(artist)}</text>
  </svg>`;
